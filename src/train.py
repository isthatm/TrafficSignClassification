...
train_images, train_labels  = utils.pickle_data(file = './Data/primary32_train_dataset')
test_images, test_labels  = utils.pickle_data(file = './Data/primary32_test_dataset')
val_images, val_labels  = utils.pickle_data(file = './Data/primary32_val_dataset')
...
# Create log dict and reporter object
report_writer = utils.Reporter(f"./{log_dir}/model_log.xlsx")
data = {"Epoch": '',
        "Batch ID": '', 
        "Loss":{"Margin": '',
                "Recon": '',
...
def train(model, train_loader, optimizer, device, epoch):
    ...
    for batch_idx, (x_batch, y_batch) in enumerate(train_loader):
        ...
        margin_loss, recon_loss, total_loss = model.loss_fn(recons_img, y_batch)
        total_loss.backward()
        ...
        if (batch_idx+1) % 5 == 0:
            ...
            report_writer.record(data)
            print('Epoch: {}, Batch: {}/{}, Loss: {:.3f}'
                    .format(epoch, batch_idx+1, len(train_loader)//batch_size, total_loss.item()))
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):  
        ...
        val_loss, val_accuracy = test(model, val_loader, device_as_str) # validate the model at each epoch
