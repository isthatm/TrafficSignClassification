...
class CapsLayers(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.num_routing_nodes is not None:
            weights = self.weights.tile(inputs.size(0), 1, 1, 1, 1) # [B, routing_nodes, #digit_caps, digit_caps_chns, prim_caps_chns]
            b_ij = self.b.tile(inputs.size(0), 1, 1, 1, 1) # [B, routing_nodes, #digit_caps, 1, 1]
            inputs = inputs.tile(1, 1, self.num_capsules, 1, 1) # [B, routing_nodes, #digit_caps, prim_caps_channels, 1]

            # u_hat = [batch, routing_nodes, #digit_caps, digit_caps_chns, 1]
            ...
            for i in range(self.num_iterations):
                ...
                # v_j OR outputs = [batch, 1 -> num_routing_nodes, #digit_caps, digit_caps_chns, 1]
            ...
            outputs = torch.cat(outputs, dim=1) # [batch, num_prim_caps, H, W, prim_channels]
            outputs = outputs.view(outputs.size(0), -1, outputs.size(4)) # u(i) = [batch, num_prim_caps*prim_caps_2D_size, prim_caps_output_dimension]
            outputs = squash(outputs)[:, :, None, :, None]
        
        # prim_outputs = [batch, num_prim_caps*prim_caps_2D_size, 1, prim_caps_dims, 1]
        # digit_outputs = [batch, 1, num_digit_caps (OR num_class), digit_caps_dims, 1]
        return outputs            
...
class CapsNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
                ...
                nn.Linear(576, 1600),
                nn.ReLU(inplace=True),
                nn.Linear(1600, 32*32*img_channels), # TODO: The output shape of this transformation is currently hard-coded
                nn.Sigmoid(),
            )
    ...
    def forward(self, images, labels=None): # labels should be applied `one_hot` function
        ...
        assert list(digit_caps_outputs.size()) == [self.batch_size, num_class, digit_channels, 1]

        v_norm = squash(digit_caps_outputs, axis=-2, squash=False) # [batch, num_classes, 1, 1]
        v_prob = F.softmax(v_norm, dim=1)

        self.img = images
        self.v_norm = v_norm.squeeze()
        ...
        masked_v = (labels[:, :, None, None] * digit_caps_outputs).view(self.batch_size, -1) 

        # Reconstruction
        reconstructed_img = self.decoder(masked_v)
    ...
    def loss_fn(self, reconstructed_img, labels):
        ...
        # original_img = [batch size, flatten image (pixels are flatten into arrays) e.g. 32x32xchannels]
